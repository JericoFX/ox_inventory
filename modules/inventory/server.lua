		...
		if stash then
			...
			if stash.owner then
				if stash.owner == true then
					owner = data.owner or (player and player.owner)
				else
					owner = stash.owner
				end
			end
...
exports('GetInventoryItems', function(inv, owner)
	local inventory = getInventory(inv, owner)
	return inventory and inventory.items
end)
...
local function getItems()
	if not Items then
		...
		if not Items then
			error('Failed to load Items module')
		end
	end
	return Items
end
...
function Inventory.SlotWeight(item, slot, ignoreCount)
	...
	if item.ammoname and slot.metadata.ammo then
		local ammoItem = getItems()(item.ammoname)
		local ammoWeight = ammoItem and ammoItem.weight
	...
	if slot.metadata.components then
		for i = #slot.metadata.components, 1, -1 do
			local componentItem = getItems()(slot.metadata.components[i])
			local componentWeight = componentItem and componentItem.weight
...
function Inventory.Save(inv)
	...
	for k, v in pairs(inv.items) do
		if not getItems().UpdateDurability(inv, v, getItems()(v.name), nil, os.time()) then
			n += 1
			buffer[n] = {
				name = v.name,
				count = v.count,
...
local function generateItems(inv, invType, items)
	...
	for i = 1, #items do
		local v = items[i]
		local item = getItems()(v[1])
		if not item then
			warn('unable to generate', v[1], 'item does not exist')
		else
			local metadata, count = getItems().Metadata(inv, item, v[3] or {}, v[2])
...
function Inventory.Load(id, invType, owner)
	...
	if result then
		...
		for _, v in pairs(result) do
			local item = getItems()(v.name)
			if item then
				v.metadata = getItems().CheckMetadata(v.metadata or {}, item, v.name, ostime)
...
function Inventory.GetItem(inv, item, metadata, returnsCount)
	if type(item) ~= 'table' then item = getItems()(item) end
	...
	if item then
		...
		if inv then
			...
			for _, v in pairs(inv.items) do
				if v.name == item.name and (not metadata or table.contains(v.metadata, metadata)) and not getItems().UpdateDurability(inv, v, item, nil, ostime) then
					count += v.count
				end
			end
		end
...
function Inventory.ContainerWeight(container, metaWeight, playerInventory)
	...
	container.weight = getItems()(container.name).weight
...
function Inventory.GetCurrentWeapon(inv)
	...
	if inv and inv.player then
		...
		if weapon and getItems()(weapon.name).weapon then
			return weapon
		end
...
function Inventory.GetSlot(inv, slotId)
	...
	if slot and not getItems().UpdateDurability(inv, slot, getItems()(slot.name), nil, os.time()) then
		return slot
	end
end
...
function Inventory.SetDurability(inv, slotId, durability)
	...
	getItems().UpdateDurability(inv, slot, getItems()(slot.name), durability)
...
function Inventory.SetMetadata(inv, slotId, metadata)
	...
	if metadata.durability ~= slot.metadata.durability then
		getItems().UpdateDurability(inv, slot, item, metadata.durability)
	else
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if not inv or not inv.slots then return false, 'invalid_inventory' end
	...
	if slot then
		...
		slotMetadata, slotCount = getItems().Metadata(inv.id, item, metadata and table.clone(metadata) or {}, count)
	...
	if not toSlot then
		...
		for i = 1, inv.slots do
			...
			elseif not item.stack and not slotData then
				...
				count -= 1
				slotMetadata, slotCount = getItems().Metadata(inv.id, item, metadata and table.clone(metadata) or {},
					count)
...
local function dropItem(source, playerInventory, fromData, data)
	...
	toData.weight = Inventory.SlotWeight(getItems()(toData.name), toData)
	...
	fromData.weight = Inventory.SlotWeight(getItems()(fromData.name), fromData)
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromData then
			...
			elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
				...
				toData.count += data.count
				fromData.count -= data.count
				local toSlotWeight = Inventory.SlotWeight(getItems()(toData.name), toData)
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					local fromSlotWeight = Inventory.SlotWeight(getItems()(fromData.name), fromData)
					...
					if not sameInventory then
						...
						if container then
							if toContainer and containerItem then
								...
								local isAllowed = exports.ox_inventory:ValidateItemForContainer(containerItem.name,
									fromData.name)

								if not isAllowed then
									return false, 'item_not_allowed_in_container'
								end
							end
						end
...
function Inventory.GetSlotForItem(inv, itemName, metadata)
	local inventory = Inventory(inv)
	local item = getItems()(itemName) --[[@as OxServerItem?]]
...
function Inventory.GetSlotWithItem(inv, itemName, metadata, strict)
	...
	for _, slotData in pairs(inventory.items) do
		if slotData and slotData.name == item.name and (not metadata or tablematch(slotData.metadata, metadata)) then
			if not getItems().UpdateDurability(inventory, slotData, item, nil, os.time()) then
				return slotData
			end
		end
	end
...
function Inventory.GetSlotIdWithItem(inv, itemName, metadata, strict)
	local slotWithItem = Inventory.GetSlotWithItem(inv, itemName, metadata, strict)
	return slotWithItem and slotWithItem.slot
end
...
local function prepareInventorySave(inv, buffer, time)
	...
	for k, v in pairs(inv.items) do
		if not getItems().UpdateDurability(inv, v, getItems()(v.name), nil, time) and shouldSave then
			n += 1
			buffer[n] = {
				name = v.name,
				count = v.count,
...
RegisterServerEvent('ox_inventory:closeInventory', function()
	...
	if inventory and inventory.open then
...
local function updateWeapon(source, action, value, slot, specialAmmo)
	...
	if type == 'table' and action == 'component' then
		...
		if weapon and weapon.metadata then
			local item = getItems()(weapon.name)
			...
			if action == 'load' and weapon.metadata.durability > 0 then
				local ammo = getItems()(weapon.name).ammoname
			...
			elseif action == 'ammo' then
				...
				elseif value < weapon.metadata.ammo then
					local durability = getItems()(weapon.name).durability *
			...
			elseif action == 'melee' and value > 0 then
				weapon.metadata.durability = weapon.metadata.durability -
					((getItems()(weapon.name).durability or 1) * value)
			end
